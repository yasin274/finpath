...
export const prisma = new PrismaClient({
  adapter,
  // В dev показываем предупреждения и ошибки; SQL включайте точечно,
...
const CAUSE_HINTS: Record<string, string> = {
  ...
  EACCES: 'Соединение заблокировано системой (firewall/политика безопасности).',
  '28P01': 'Неверный логин или пароль в DATABASE_URL.',
  '3D000': 'Указанной базы данных не существует — создайте её.',
  '28000': 'Пользователю запрещено подключаться (см. pg_hba.conf).',
  '42P01': 'Таблиц Finpath нет в базе — примените prisma/migrations/0_init/migration.sql.',
};

/**
 * Достаёт человекочитаемую причину сбоя из ошибки Prisma.
 *
 * Prisma 7 оборачивает ошибку драйвера в своё «Invalid `prisma.$queryRaw()`
 * invocation» с пустым телом, а настоящая причина лежит глубже в цепочке
 * `cause`. Без этого разбора диагностика подключения превращается в гадание.
 */
...
/**
 * Проверка соединения с БД на старте.
 *
