...
/** Полезная нагрузка нашего access-токена. */
export interface AccessTokenPayload {
  /** id пользователя (стандартное поле JWT `sub`) */
  sub: string;
  email: string;
}
 ...
 *
 * Это защита от «algorithm confusion»: без ограничения списка алгоритмов
 * злоумышленник подсовывает токен с заголовком `alg: none` или подменяет
 * HS256 на RS256 и заставляет библиотеку принять подделку. Классическая дыра
 * в JWT-аутентификации, закрывается ровно одной строкой.
 */
const ALGORITHM = 'HS256' as const;

/**
 * JWT_EXPIRES_IN приходит из .env строкой, а @types/jsonwebtoken ждёт здесь
 * литеральный тип `StringValue` из пакета `ms` ('7d' | '24h' | ...). Произвольная
 * строка в него не подходит, поэтому приводим тип явно — значение проверит `ms`
 * в рантайме и упадёт на старте, если оно кривое.
 */
const expiresIn = env.JWT_EXPIRES_IN as SignOptions['expiresIn'];

/** Подписывает access-токен для пользователя. */
 ...
 *
 * Наружу отдаёт только 401 без деталей: клиенту незачем знать, токен просрочен
 * или подпись не сошлась — действие в обоих случаях одно, залогиниться заново.
 */
