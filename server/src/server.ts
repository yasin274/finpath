...
async function bootstrap(): Promise<void> {
  console.log('Запуск Finpath API');
  ...
  if (db.ok) {
    ...
    console.warn('   Сервер поднимется, но запросы к БД будут падать (dev-режим).');
    console.warn('   Проверьте DATABASE_URL в .env и примените prisma/migrations/0_init/migration.sql.');
  }

  const server = app.listen(env.PORT, () => {
    console.log(`   Сервер слушает http://localhost:${env.PORT}`);
    console.log(`   Дашборд:      http://localhost:${env.PORT}/dashboard.html`);
   ...
   * Postgres рано или поздно упирается в max_connections — особенно больно,
   * когда база общая с другими проектами.
   */
  const shutdown = async (signal: string): Promise<void> => {
    ...
    setTimeout(() => {
      console.error('Не дождался закрытия соединений, выхожу принудительно');
      process.exit(1);
    }, 10_000).unref();
  };
